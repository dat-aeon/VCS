...
$(function() {
	...
	if (window.location.port === '8082') {
		connection = new WebSocket('ws://' + window.location.hostname + ':8084')
	} else {
		connection = new WebSocket('wss://' + window.location.hostname
				+ '/vcschat')
	}
	console.log(connection);
	//	var connection = new WebSocket('wss://ass.aeoncredit.com.mm/vcschat');
	//var connection = new WebSocket('ws://10.1.9.69:8084');
	...
	connection.onmessage = function(message) {
		...
		} else if(json.type === 'searchCustomer'){
			
			
			var allRoom = new Array();
			for (var i = 0; i < json.data.length; i++) {
				var phoneNoVal = json.data[i].phone_no;
				var nrcNoVal = json.data[i].nrc_no;

				var msgHeaderVal = json.data[i].name + " " + "(" + phoneNoVal
						+ ")";
				autocompleteNrcNoTemp = nrcNoVal;
				clickRoom(phoneNoVal, msgHeaderVal);
			}
	...
	function searchCustomer(){
		var phoneNo = document.getElementById("search").value;
		connection.send("searchCustomer:" + phoneNo+ "op_send_flag:" + 0);
		console.log(phoneNo);
	}
	...
	function addRooms(customer_id, name, phone_no, count, nrc_no) {
		window.inboxChatList.innerHTML = window.inboxChatList.innerHTML
				...
				+ '" onclick="clickRoom(\'' + phone_no + '\',\'' + name + ' ('
				+ phone_no + ')' + ' (' + nrc_no + ')' + '\')">'
				+ '<div class="user_info">' + '<div class="user_name">' + name
				+ ' (' + phone_no + ') </div>' + '<div class="phone_no">'
				+ nrc_no + '</div>' + '</div>' + '<div class="count">' + count
				+ '</div>' + '</div>';
