...
import mm.aeon.com.ass.base.dto.passwordInfoSelectForUpdate.PasswordInfoSelectForUpdateReqDto;
import mm.aeon.com.ass.base.dto.passwordInfoSelectForUpdate.PasswordInfoSelectForUpdateResDto;
...
public class PasswordChangeController extends AbstractController
        implements IControllerAccessor<PasswordChangeFormBean, PasswordChangeFormBean> {

    private ApplicationLogger applicationLogger = new ApplicationLogger();

    @Override
    public PasswordChangeFormBean process(PasswordChangeFormBean formBean) {
        ...
        PasswordInfoSelectForUpdateReqDto selectForUpdateReqDto = new PasswordInfoSelectForUpdateReqDto();
        String hashOldPassword = PasswordEncoder.encode(formBean.getPasswordChangeHeaderBean().getOldPassword());
        selectForUpdateReqDto.setOldPassword(hashOldPassword);
        selectForUpdateReqDto.setUserId(CommonUtil.getLoginUserId());

        try {
            PasswordInfoSelectForUpdateResDto selectForUpdateResDto =
                    (PasswordInfoSelectForUpdateResDto) this.getDaoServiceInvoker().execute(selectForUpdateReqDto);
            if (selectForUpdateResDto == null) {
                msgBean =
                        new MessageBean(MessageId.ME1040, DisplayItemBean.getDisplayItemName(DisplayItem.OLD_PASSWORD));
                msgBean.addColumnId(DisplayItem.OLD_PASSWORD);
    ...
    private PasswordChangeFormBean setErrorMessage(PasswordChangeFormBean formBean, String serviceStatus) {
    ...
    private boolean isValidData(PasswordChangeFormBean formBean) {
        boolean isValid = true;
        // boolean isCheckCode = false;
        MessageBean msgBean;

        if (InputChecker.isBlankOrNull(formBean.getPasswordChangeHeaderBean().getOldPassword())) {
            msgBean = new MessageBean(MessageId.ME0003, DisplayItemBean.getDisplayItemName(DisplayItem.OLD_PASSWORD));
            msgBean.addColumnId(DisplayItem.OLD_PASSWORD);
            msgBean.setMessageType(MessageType.ERROR);
            formBean.getMessageContainer().addMessage(msgBean);
            isValid = false;
